...
phones = [f"Phone {i}" for i in range(1, 101)]  # List of 100 phones (this can be a database query)
items_per_page = 12
...
def index():
    ...
    total_pages = len(phones) // items_per_page
    if len(phones) % items_per_page > 0:
        total_pages += 1

    # Slice the data to only show the phones for the current page
    start = (page - 1) * items_per_page
    end = start + items_per_page
    phones = phones[start:end]


    return render_template('index.html',total_pages=total_pages, current_page=page, images=images, phones=phones, current_user=user)
